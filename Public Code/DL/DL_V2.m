...
% Version 2 - PB only, more healthy data & option for binary fault codes
%----------------------------------------------------------------------------------------------------------------------------------------%
...
% Initialize tables
num_signals_PB = 10*20*N_Split;         % 7 Signals * 20 runs * N_Split Samples (for Paderborn)
...
faultCode(1:4*20*N_Split) = 0;
faultCode(4*20*N_Split+1:7*20*N_Split) = 1;
faultCode(7*20*N_Split+1:10*20*N_Split) = 2;

% % Alternate Binary faultCode - 0,1 = normal, faulty
% faultCode(1:4*20*N_Split) = 0;
% faultCode(4*20*N_Split:10*20*N_Split) = 1;
...
TrainingFolderName = 'DL_V2_2_Training';
...
for k = 1:height(dataTable)
    ...
    SignaltoScalogram(SampleVib,TrainingFolderName,k,num2str(SampleCode))
...
% Test_faultCode(1:20*N_Split) = 0;
% Test_faultCode(20*N_Split+1:4*20*N_Split) = 1;
...
for k = 1:height(TestdataTable)
    ...
    SignaltoScalogram(SampleVib,TestFolderName,k,num2str(SampleCode))
