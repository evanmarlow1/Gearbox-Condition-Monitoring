...
numRows = 2*10*8*N_Split;       % 2 fault codes * 10 signals * 8 sensors * N_Split subsignals

FeatureTable = table('Size', [numRows 4],'VariableTypes',{'double','double','double','double'} ,'VariableNames', {'faultCode', 'FM0', 'ER', 'RMS'});
...
HealthySignalTable = cell(10*8,N_Split);
FaultySignalTable = cell(10*8,N_Split);

indexoffset = 10*8*N_Split;
...
for i = 1:10
    ...
    for j = 1:8
        ...
        indexStart = ((i-1) * 8 + (j-1)) * N_Split + 1;
       ...
       indexStart = ((i-1) * 8 + (j-1)) * N_Split + 1 + indexoffset;
       indexEnd = indexStart + N_Split - 1;
...
eightsignals = N_Split*8*8;

Training_Healthy = FeatureTable(1:eightsignals,:);
Test_Healthy = FeatureTable(eightsignals+1:halflength,:);

Training_Faulty = FeatureTable(halflength + 1: halflength + eightsignals,:);
Test_Faulty = FeatureTable(halflength + eightsignals + 1 : end,:);
