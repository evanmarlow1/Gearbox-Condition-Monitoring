...
% Version 3 - All Key Signals, 2 Classes, PB and CWRU, Split data
%----------------------------------------------------------------------------------------------------------------------------------------%
...
%Change split length to desired.
N_Split = 15;
N_Split_CWRU = 15;
...
CWRUTestTable = table(CWRUTables, CWRUTacho, CWRUFault, 'VariableNames', {'Vibration','Tachometer','faultCode'});
...
% % MultiClass Fault Codes
% multiFaultCode = zeros(length(faultCode),1);
% 
% % We define Normal, IR, OR as 0,1,2
% multiFaultCode(20*N_Split+1 : 4*20*N_Split) = 1;
% multiFaultCode(4*20*N_Split+1 : 7*20*N_Split) = 2;
% multiFaultCode(7*20*N_Split+1 : 4*)
