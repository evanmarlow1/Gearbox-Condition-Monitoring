...
T = table('Size',[height(dataTable) 4],'VariableTypes',{'cell','cell','cell','cell'},'VariableNames',{'TSA','Diff','Reg','Res'});

Fav = BPFO_PB;

for i = 1:height(dataTable)
    
    signal = dataTable{i,'Vibration'}; % Accessing cell value using {}
    signal = signal{1}; % Extracting the value from the cell array

    TSA = tsa(signal,1/Fav);
    res = tsaresidual(TSA,Fav*60, OrderList);
    diff = tsadifference(TSA, Fav*60, OrderList);
    reg = tsaregular(TSA, Fav*60, OrderList);
...
featureData = gearMetrics.("FM4");
faultCode = dataTable.faultCode;
...
freqs = Fr*[BPFO-1 BPFO BPFI-1 BPFO+1 BPFI BPFI+1]; 

amplitudes = table('Size',[height(dataTable) 6],'VariableTypes',{'cell','cell','cell','cell','cell','cell'},'VariableNames',{'BPFOL','BPFO','BPFIL','BPFOH','BPFI','BPFIH'});
...
for i = 1:height(dataTable)
    ...
    signal = dataTable{i,'Vibration'}; % Accessing cell value using {}
    signal = signal{1}; % Extracting the value from the cell array
    vib = signal.Vibration;

    out = bandpassAmplitudes(vib,Fs,freqs);

    amplitudes{i,'BPFOL'} = {out(1)};
    amplitudes{i,'BPFO'} = {out(2)};
    amplitudes{i,'BPFIL'} = {out(3)};
    amplitudes{i,'BPFOH'} = {out(4)};
    amplitudes{i,'BPFI'} = {out(5)};
    amplitudes{i,'BPFIH'} = {out(6)};
...
variable = amplitudes.BPFIL;
variable = cell2mat(variable);


figure;
gscatter(1:numel(variable), variable, dataTable.faultCode, 'br', '.', 10); % Scatter plot colored by faultCode
