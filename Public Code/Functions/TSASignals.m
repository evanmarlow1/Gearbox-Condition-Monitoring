function [TSA,TSA_RES,TSA_DIFF,TSA_REG] = TSASignals(Data,Sample_Freq, Averaging_Freq, OrderList)
    ...
    for i = 1:DataRows
        ...
        TSA_RES_Row = tsaresidual(Data(i,:),Sample_Freq,Averaging_RPM,OrderList);
        TSA_RES(i,:) = TSA_RES_Row(1:length(TSA_RES));

        TSA_DIFF_Row = tsadifference(Data(i,:),Sample_Freq,Averaging_RPM,OrderList);
        TSA_DIFF(i,:) = TSA_DIFF_Row(1:length(TSA_DIFF));

        TSA_REG_Row = tsaregular(Data(i,:),Sample_Freq,Averaging_RPM,OrderList);
